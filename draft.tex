...
\begin{document}
	...
	\par
	Yang et al. \cite{yang_robust_2024} demonstrate a multi-feature comparison, as well as a
	...
	In-the-Wild \cite{muller_does_2022}, presenting their results for each features, as well as
	...
	experiments, which were performed as well on the dataset of the ASV2022 Challenge
	\cite{yi_add_2024}.
	...
	\label{sec:method}
		...
		as well as the way in which explanations were created and summarized.
			...
			clarification is in the following section.
			\par
			...
			into the terminus model at the end, which produces the final result.
			Figure \ref{fig:submodel} offers a deeper look into the general architecture of a
			singular sub-model in our method. The sub-models begin with an input layer, taking
			either a vector or matrix depending on the dimensionality of the feature to which the
			model is tailored. The inputs are then passed into alternating convolutional and
			pooling layers, in order to increase the localized pattern detection capability within
			the sub-models. Figure \ref{fig:term_model} is a generalized representation of the
